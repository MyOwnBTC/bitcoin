    ...
    platformStyle(_platformStyle)
{
    ...
    // Always flash for maximum safety - same as main window
    std::string block1hash = gArgs.GetArg("-block1hash", "");
    const std::string MAIN_CHAIN_BLOCK1 = "00000000839a8e6886ab5951d76f411475428afc90947ee320161bbf18eb6048";
    
    // Log warning regardless of block1hash value for safety
    if (!block1hash.empty() && block1hash != MAIN_CHAIN_BLOCK1) {
        LogPrintf("WARNING: Node window running with fork chain block1hash: %s\n", block1hash);
    }

    // Create flash timer for node window safety - Orange 1s, Normal 5s cycle
    QTimer* nodeFlashTimer = new QTimer(this);
    bool* isNodeOrange = new bool(false);
    QString* nodeOriginalStyleSheet = new QString(this->styleSheet()); // Store original style
    
    connect(nodeFlashTimer, &QTimer::timeout, [this, isNodeOrange, nodeOriginalStyleSheet, nodeFlashTimer]() {
        *isNodeOrange = !*isNodeOrange;
        if (*isNodeOrange) {
            // Orange warning state for node window (1 second)
            this->setStyleSheet("* { background-color: #FF8C00 !important; color: #FFFFFF !important; }");
            QTimer::singleShot(1000, [this, nodeFlashTimer, nodeOriginalStyleSheet]() {
                // Return to normal state (5 seconds)
                this->setStyleSheet(*nodeOriginalStyleSheet);
                nodeFlashTimer->start(5000); // Wait 5 seconds before next orange flash
            });
        }
    });
    
    nodeFlashTimer->start(1000); // Start with 1 second orange flash
...
void RPCConsole::updateDetailWidget()
{
    ...
    // Show peer's block1hash if available
    if (!stats->nodeStats.m_block1hash.IsNull()) {
        ui->peerBlock1Hash->setText(QString::fromStdString(stats->nodeStats.m_block1hash.ToString()));
    } else {
        ui->peerBlock1Hash->setText(tr("N/A"));
    }
...
void RPCConsole::updateWindowTitle()
{
    ...
    if (chain == ChainType::MAIN) {
        // Always add warning for main chain safety
        QString title = "⛔ WARNING: This is potentially NOT the Original Bitcoin Canonical Mainnet!!! - Node window";
        this->setWindowTitle(title);
        return;
    }

    const QString chainType = QString::fromStdString(Params().GetChainTypeString());
    const QString title = QString("⛔ WARNING: This is potentially NOT the Original Bitcoin Canonical Mainnet!!! - Node window - [%1]").arg(chainType);
    this->setWindowTitle(title);
}
