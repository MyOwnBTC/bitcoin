    ...
    m_network_style(networkStyle)
{
    ...
    // Always flash for maximum safety
    ...
    if (!block1hash.empty() && block1hash != MAIN_CHAIN_BLOCK1) {
        LogPrintf("WARNING: Running with fork chain block1hash: %s\n", block1hash);
    } else if (!block1hash.empty()) {
        LogPrintf("INFO: Running with main chain block1hash verification\n");
    } else {
        LogPrintf("WARNING: Running without block1hash verification\n");
    }
    
    // Always create flash timer for safety - Orange 1s, Normal 5s cycle
    QTimer* flashTimer = new QTimer(this);
    bool* isOrange = new bool(false);
    QString* originalStyleSheet = new QString(this->styleSheet()); // Store original style
    
    connect(flashTimer, &QTimer::timeout, [this, isOrange, originalStyleSheet, flashTimer]() {
        *isOrange = !*isOrange;
        if (*isOrange) {
            // Orange warning state (1 second)
            this->setStyleSheet("* { background-color: #FF8C00 !important; color: #FFFFFF !important; }");
            QTimer::singleShot(1000, [this, flashTimer, originalStyleSheet]() {
                // Return to normal state (5 seconds)
                this->setStyleSheet(*originalStyleSheet);
                flashTimer->start(5000); // Wait 5 seconds before next orange flash
            });
        }
    });
    
    flashTimer->start(1000); // Start with 1 second orange flash
...
void BitcoinGUI::updateWindowTitle()
{
    ...
    // Always add warning for safety
    window_title += " ⛔ WARNING: This is potentially NOT the Original Bitcoin Canonical Mainnet!!!";
