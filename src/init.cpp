...
namespace {
    static uint256 g_configured_block1_hash;
    static bool g_disable_chain_identity_check;
}

const uint256& GetConfiguredBlock1Hash() {
    return g_configured_block1_hash;
}

bool GetDisableChainIdentityCheck(){
    return g_disable_chain_identity_check;
}
...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-block1hash=<hex>", strprintf("block 1 hash"), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-disablechainidentitycheck", strprintf("Disable chain identity verification (for testing only)"), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY,
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    std::string block1HashStr = args.GetArg("-block1hash", "");
    if (block1HashStr.empty()) {
        return InitError(_("Error: -block1hash must be specified. This identifies which chain to connect to.\n"));
    }
    if (!IsHex(block1HashStr)) {
        return InitError(strprintf(_("Error: -block1hash must be a hexadecimal string: '%s'"), block1HashStr));
    }
    if (block1HashStr.length() != 64) {
        return InitError(strprintf(_("Error: -block1hash must be exactly 64 characters (got %d): '%s'"), 
                                 block1HashStr.length(), block1HashStr));
    }
    // Check if this is mainnet block hash
    const std::string MAINNET_BLOCK1_HASH = "00000000839a8e6886ab5951d76f411475428afc90947ee320161bbf18eb6048";
    if (block1HashStr == MAINNET_BLOCK1_HASH) {
        return InitError(_("Error: Block1hash 00000000839a8e6886ab5951d76f411475428afc90947ee320161bbf18eb6048 is not supported."));
    }
    
    uint256 block1Hash{ParseHex(block1HashStr)};
    if (block1Hash.IsNull()) {
    	return InitError(_("Error: -block1hash cannot be zero, or failed to parse"));
    }
    std::reverse(block1Hash.begin(), block1Hash.end());
    g_configured_block1_hash = block1Hash;
    LogPrintf("Chain selector: Configured for chain with block 1 hash %s\n", block1Hash.ToString());

    if (args.GetBoolArg("-disablechainidentitycheck", false)) {
        g_disable_chain_identity_check = true;
    }
