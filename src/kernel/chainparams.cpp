...
public:
    ...
    CMainParams(const std::string& block1hash) {
        InitMainParams(block1hash);
    }
    
private:
    void InitMainParams(const std::string& block1hash) {
     	bool main=false;
	// Check block1hash parameter to determine chain configuration  
	// Main chain vs fork chain determination
	const std::string MAIN_CHAIN_BLOCK1_HASH = "00000000839a8e6886ab5951d76f411475428afc90947ee320161bbf18eb6048";
	if (block1hash == MAIN_CHAIN_BLOCK1_HASH) 
		main=true;
	...
	if(main){
        ...
        // DNS seeds are now loaded from configuration instead of being hardcoded
	...
	if (main) {
	    // Use Bitcoin Core official release defaults
	    ...
	    // Use custom chain defaults (your current values)
	    chainTxData = ChainTxData{
	        .nTime    = 0,
	        .tx_count = 0,
	        .dTxRate  = 0,
...
std::unique_ptr<const CChainParams> CChainParams::Main(const std::string& block1hash)
{
    return std::make_unique<const CMainParams>(block1hash);
}
