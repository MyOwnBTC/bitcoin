...
void PeerManagerImpl::PushNodeVersion(CNode& pnode, const Peer& peer)
{
    ...
    // Get our configured block1hash to send to peer
    uint256 block1hash = GetConfiguredBlock1Hash();
    ...
    MakeAndPushMessage(pnode, NetMsgType::VERSION, PROTOCOL_VERSION, my_services, nTime,
            ...
            nonce, strSubVersion, nNodeStartingHeight, tx_relay, block1hash);
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        // Try to read block1hash if peer sends it (new protocol extension)
        uint256 peer_block1hash;
        if (!vRecv.empty()) {
            try {
                vRecv >> peer_block1hash;
                // Store the peer's block1hash
                pfrom.m_block1hash = peer_block1hash;
                LogDebug(BCLog::NET, "peer=%d sent block1hash=%s\n", pfrom.GetId(), peer_block1hash.ToString());
            } catch (...) {
                // Old peer that doesn't send block1hash, ignore
                LogDebug(BCLog::NET, "peer=%d didn't send block1hash (old version)\n", pfrom.GetId());
            }
        }
