...
{
    // Allow solo mining when maxconnections=0 and only genesis block exists
    if (gArgs.GetIntArg("-maxconnections", DEFAULT_MAX_PEER_CONNECTIONS) == 0) {
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool min_pow_checked)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
	 ...
	 if (pindexPrev && pindexPrev->nHeight == 0) {
		const uint256& expectedHash = GetConfiguredBlock1Hash();
		uint256 blockHash = block.GetHash();

		if (blockHash != expectedHash) {
		    	LogPrintf("Reject wrong chain block at height 1 (expected %s, got %s)?\n", expectedHash.ToString(), blockHash.ToString());
			if (!GetDisableChainIdentityCheck()) {
			    LogPrintf("Rejecting wrong chain block at height 1: expected %s, got %s\n", expectedHash.ToString(), blockHash.ToString());
			    return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "wrong-chain-identity", "Block belongs to different chain");
			}
		}
	 }
